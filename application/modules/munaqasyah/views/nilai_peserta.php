...
<div class="row">
    ...
    <?php $is_done = $this->db->query('
                SELECT t.peserta_munaqasyah_id, a.kriteria_nilai_id, sum(t.nilai) AS nilai
    ...
    <?php if(count($is_done) == 1):?>
        <div class="col-lg-3 col-xs-6">
            <?php $bg_small_box = ($is_done->nilai < 7.5) ? 'bg-yellow' : 'bg-aqua';?>
            <!-- small box -->
            <div class="small-box <?=$bg_small_box?> ">
            <div class="inner">
                <h3> <i class="fa fa-check"></i> </h3>

                <p><?= $k->display_name?></span></p>
            </div>
            <div class="icon">
                <span><?=$is_done->nilai?>
            </div>
